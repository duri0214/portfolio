...
class KokkaiPipeline:
    ...
    def _split_by_agenda(record: MeetingRecord) -> list[tuple[str, list[SpeechRecord]]]:
        ...
        [ドメイン粒度での分割の意義: 思考対象への変換]
        国会は制度的に「議題」という単位で議論が進められます。この事実に即した粒度で
        データを分割し、RAGフレンドリな構造へと再構成することには以下の価値があります：

        1. 検索精度のドメイン最適化:
           「どの論点で語ったか」というドメイン情報（議題名）をメタデータとして持つことで、
           意味検索の適合度が劇的に向上します。
        2. 政治的文脈の保持:
           発言がどの制度（法律案など）や主題の下で行われたかを、データのバッチ境界として
           物理的に定義することで、LLMによる高度な文脈理解を助けます。
        3. 閲覧性のドメイン統合:
           UI上で「制度としての議題」ごとに構造化されることで、ユーザーにとって
           馴染みのある形式（＝思考しやすい形式）で情報を提供できます。

        この工程は、単なるテキスト分割ではなく、国会の議論構造をデータ上に復元する「意味的再構成」です。
        """
        agendas = []
