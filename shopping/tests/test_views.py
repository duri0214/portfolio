...
class TestView(TestCase):
    ...
    def test_get_store_planning_page_200(self):
        ...
        self.assertContains(response, "出店判断サマリー")
        self.assertContains(response, "意思決定支援")
        self.assertContains(response, "評判・口コミ")
        self.assertContains(response, "周辺人口")
        self.assertContains(response, "来店タイプ仮説")
        self.assertContains(response, "通りすがり型・目的来店型・地域密着型")
        self.assertContains(response, "比較地域")
