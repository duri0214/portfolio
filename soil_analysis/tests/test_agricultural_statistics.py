...
class AgriculturalRiskReportViewTest(TestCase):
    ...
    def test_report_view_displays_latest_risk_report(self):
        ...
        self.assertContains(response, "全国比 0.0%")
        self.assertContains(response, "全国差 -8.0pt")
        self.assertContains(response, "全国差 -2.0pt")
        ...
        self.assertContains(
            response, "分類は圃場1筆ごとではなく経営体全体の経営耕地面積で決まります。"
        )
        self.assertContains(
            response,
