...
class TestLand(TestCase):
    def test_land_initialization(self):
        ...
        expected_coord = LandLocation(
            "137.6491060553256,34.74361968398954", "ススムA3の中心点"
        )

        land = LandLocation(coord_str, name)

        self.assertEqual(name, land.name)
        self.assertAlmostEqual(
            expected_coord.latitude, land.center.latitude, delta=0.0001
        )
        self.assertAlmostEqual(
            expected_coord.longitude, land.center.longitude, delta=0.0001
        )
