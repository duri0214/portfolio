...
class FieldDefinitionTests(SimpleTestCase):
    ...
    def test_alias_resolution_returns_expected_keys(self):
        self.assertEqual(FIELD_ALIAS_TO_KEY.get(normalize_text("NH4-N")), "nh4n")
        self.assertEqual(FIELD_ALIAS_TO_KEY.get(normalize_text("no3n")), "no3n")
        self.assertEqual(FIELD_ALIAS_TO_KEY.get(normalize_text(" P2O5 ")), "p2o5")
        self.assertEqual(
            FIELD_ALIAS_TO_KEY.get(normalize_text("リン酸吸収係数")),
            "phosphorus_absorption",
        )
        self.assertIsNone(FIELD_ALIAS_TO_KEY.get(normalize_text("unknown-column")))

    def test_major_display_labels_and_units(self):
        by_key = {field.key: field for field in REPORT_FIELD_DEFINITIONS}
