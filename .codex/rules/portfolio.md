...
- 変更は必ずローカル作業ツリーで編集し、`git add`、`git commit`、`git push`、PR 作成の通常フローで反映する。
...
- PRレビューコメントの指摘を修正した場合は、PR全体への新規コメントではなく、原則として元のレビューコメントに対する返信として `gh` でコメントする。レビューコメント対応の詳細手順は `review` スキルに従う。

## レスポンスエラーの可視化
- API・外部サービス・画面処理で発生したエラーを、302などのリダイレクトや成功レスポンスに置き換えて握りつぶさない。
- エラーの種類に応じたHTTPステータスを返し、画面では利用者が原因と次の対応を判断できるメッセージを表示する。
- 外部サービスの詳細なエラーはサーバーログに記録し、利用者向けメッセージには秘密情報や不要な内部情報を含めない。
...
## UI 権限制御方針
- 管理者専用機能の導線は対象アプリのトップ画面や主要な一覧画面に置き、利用者が管理画面のURLを直接入力することを前提にしない。
- 「スーパーユーザーのみ」「管理者のみ」など、特定権限だけが実行できる操作を画面に置く場合、権限のないユーザーに対してボタンや導線を消さない。
- 未ログインを含む権限のないユーザーには、原則として同じ位置に disabled 状態のボタンを表示し、機能の存在と実行不可であることを分かるようにする。
