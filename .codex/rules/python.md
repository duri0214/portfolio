...
- **実装のシンプルさ (KISS 原則) の優先**:
    - `frozen=True` や `dataclass` の機能を活用して不変性を保つことは推奨されるが、`__post_init__` で複雑な計算が必要になったり、
      `object.__setattr__` を使わなければならないような場合は、無理に適用しない。
    - `dataclass` を使うことでかえってコードが冗長になったり、直感的でなくなる場合は、**プレーンなクラスと `__init__`
      メソッドを用いたシンプルな実装を優先すること**。
