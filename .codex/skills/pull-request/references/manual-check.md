...
- 既存または追加したAPIテストで担保できる挙動は、PR本文の目検手順に長い確認コマンドとして再実装しない。目検コマンドはテストで担保できない表示確認、環境差分、外部連携、手元での最小スモーク確認に絞る。
- APIテストで担保済みの内容は、`## 自動テストでカバーできた範囲` にテスト名や確認観点を書き、目検手順では「自動テストでカバー済み」と分かるようにする。
