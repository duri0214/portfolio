...
- ユーザーが目検の写真・画面キャプチャ・実行結果を提示した場合は、同リファレンスの「ユーザー提供の目検結果を反映する場合」を読んでからPR本文を更新する。

## ユーザー提供の目検結果を反映する場合

- PR本文の目検チェック項目を、提示された証拠またはユーザーが明示した確認結果と照合する。
- 証拠から直接確認できる項目だけを `[ ]` から `[x]` に変更する。画面に写っていない状態、件数、再読込後の状態は推測でチェックしない。
- チェックした項目には、確認できた具体的な値・状態を必要に応じて追記し、未確認項目は `[ ]` のまま残す。
- `gh pr edit <番号> --body-file <一時ファイル>` で本文を更新し、`gh pr view <番号> --json body` でチェック状態を確認する。
- 追加の写真や結果を受け取った場合は、既存のチェックを保持し、新たに確認できた項目だけを更新する。
