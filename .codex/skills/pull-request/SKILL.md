...
- 目検の確認項目は、確認単位ごとに必ず `- [ ]` のチェックボックスで記載し、レビュアーが確認済みの項目を `[x]` に変更できる形にする。番号付きリストや通常の箇条書きだけで目検手順を書かない。
- PR作成時点で未実施の目検は `[ ]` のままにし、実施済みの自動テスト・静的チェックは `[x]` として記載する。
...
- ユーザーが目検の写真・画面キャプチャ・実行結果を提示した場合は、同リファレンスの「ユーザー提供の目検結果を反映する場合」を読んでから、PR本文の各チェック項目と一つずつ照合する。
- 写真や結果から直接確認できる項目、またはユーザーが明示した確認結果だけを `[x]` に更新し、確認できない項目は `[ ]` のまま残す。チェックした項目には必要に応じて具体的な値・状態を追記する。
- 目検結果を反映したPR本文は、`gh pr edit <番号> --body-file <一時ファイル>` で更新し、`gh pr view <番号> --json body` でチェック状態と記録内容を確認する。
