...
- pull-request スキルが呼ばれた時点で未コミット変更がある場合は、`.codex/skills/commit/SKILL.md` のチェック手順を内包してコミットし、push してから PR 作成へ進む。この場合、ユーザーは PR 作成を依頼しているため、コミット後の push 追加確認は省略してよい。
