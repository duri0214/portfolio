...
- 親チケットは、現在ブランチ名の先頭番号、現在ブランチの open PR、または会話で明示された Issue から推定する。推定元 Issue に親チケットがある場合は、その親を新規 Issue の親候補として扱う。
...
- **Parent issue**: 現在ブランチ名の先頭番号、現在ブランチの open PR、または会話で明示された Issue を確認し、その Issue に親チケットがある場合は新規 Issue の親として扱う。親候補を一意に決められる場合は追加確認なしで本文に `親チケット: #<番号>` を含め、Issue 作成後に GitHub の Sub-issues 機能で親子関係を設定する。親候補がない場合は親なしで作成する。親候補が複数あり一意に決められない場合だけユーザーに確認する。設定できない場合は、本文上の参照だけは残し、失敗理由を報告する。
