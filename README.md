...
`scripts/import_data.sh` は既存データを削除して fixture を投入するため、通常更新では実行しません。
...
商品購入デモに加えて、店舗候補の座標や e-Stat 人口統計を使った出店計画画面を提供します。
出店計画では、保存済み Google Maps レビューを半径500mの3x3グリッドへ集約して表示します。

- **ドキュメント:**
  - [shopping/README.md](shopping/README.md)
