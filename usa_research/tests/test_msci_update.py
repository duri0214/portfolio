...
class MsciUpdateIdempotencyTest(TestCase):
    ...
    def test_initial_run(
        ...
        - HTTPヘッダの Last-Modified から抽出された日付で新規レコードが作成されることを確認する。
        """
        # Setup mocks
    ...
    def test_idempotency_same_date(
        ...
        - 既に特定のReport Dateのレコードが存在する状態で、同じLast-Modifiedを持つPDFを処理する。
        ...
        # Setup mocks to return the same date in HEAD
        mock_requests_head.return_value = MagicMock(
            status_code=200, headers={"Last-Modified": "Wed, 01 May 2024 10:00:00 GMT"}
        )
        ...
        self.assertEqual(MsciCountryWeightReport.objects.count(), 1)
        self.assertIn(f"no update since {report_date}", self.out.getvalue())
    ...
    def test_new_date_update(
        ...
        """
        シナリオ3: 新規データの更新
        - DBに古い日付のレコードが存在する状態で、より新しいLast-Modifiedを持つPDFを処理する。
    ...
    @patch.dict("os.environ", {"OPENAI_API_KEY": "fake_key"})
    def test_old_date_skip(
        self, mock_llm_service, mock_pdf_reader, mock_requests_get, mock_requests_head
    ):
        """
        シナリオ4: 過去データのスキップ（逆転防止）
        - 最新レコードよりも古いLast-Modifiedを持つPDFが提示された場合。
        ...
        # PDFのLast-Modifiedが過去（2024-04-01）の場合
        mock_requests_head.return_value = MagicMock(
            status_code=200, headers={"Last-Modified": "Mon, 01 Apr 2024 10:00:00 GMT"}
        )
