...
- 「Webから取り込んで確認」: URLまたは本文をGPTで候補化し、作成直後に候補確認画面へ進む
- 「保存済み候補を確認」: 以前に作成したWeb取込候補の確認・修正・承認
...
読み仮名・用語辞書の「Webから取り込んで確認」で、公式ページのURLまたは本文を入力します。URLを指定した場合はページ本文を取得し、`OPENAI_API_KEY` で設定したGPTへ候補作成を依頼します。GPTの出力には用語、読み、説明、カテゴリ、出典URLの候補が含まれます。候補作成後は確認画面へ遷移し、確認・修正と登録承認を行ってから辞書へ登録します。
