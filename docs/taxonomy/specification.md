...
- LLM で分類候補を一度だけ生成し、`taxonomy/data/animal_taxonomy_seed.py` に構造化データとして Git 管理する。
- 投入は `python manage.py seed_taxonomy_animals` で行う。
- コマンドは `get_or_create` により再実行しても重複しない。
