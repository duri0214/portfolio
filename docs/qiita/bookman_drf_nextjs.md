...
このとき AI に「いい感じにして」だけを渡すと、汎用的な SaaS のランディングページに寄りやすい。今回なら、以下のようにプロダクト固有の文脈を渡した方が狙った方向へ近づく。

- Next.js のデフォルト HOME を、Bookman 向けの図書館システムらしい入口に変えること
- 昔の図書貸出カードをモチーフにして、少し揺れるアニメーションでノスタルジーを出すこと
- HOME の対象は主要導線に絞り、ダッシュボードや一覧画面のデザイン刷新までは広げないこと
- 「図書館を管理」は実装済みの `/branch` へ進める導線にすること
- 「本をかりる」は未実装なので、クリックできるリンクではなく disabled 表示にすること
- 図書カードは HOME 専用の飾りで終わらせず、再利用できる `LibraryCard` コンポーネントとして切り出すこと

AI に任せる範囲を「デザインの方向づけと実装のたたき台」に限定し、実装済み機能、未実装機能、今後使い回したい部品を人間側で指定する。そうしないと、見た目は整っていても、現在のアプリの状態と合わない導線が混ざりやすい。
