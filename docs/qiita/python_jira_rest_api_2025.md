# PythonでシンプルにJiraからチケット情報を取得・CRUDする2025

## はじめに

現場でJiraを使う機会が増えたので、Jira Cloud REST API v3 を Python から素朴に呼び出す形で整理します。

以前は「プロジェクト一覧を取る」「チケット一覧を取る」ところまででしたが、今回は Issue #236 の対応として、チケットの作成・取得・編集・削除までを `lib/jira/` に足しました。
...
- Issue #236: lib: jiraライブラリにCRUDをつける
...
Issue #236 の目的は、単純なCRUDで終わらせず、Jiraチケットを「意思決定ログ」として扱い、あとからスキル傾向や作業タイプを分析できる状態にすることです。今回のCRUDは、そのための入口です。

## 次にやること

- チケット取得結果をJSONスナップショットとして保存する
- 1週間分程度のチケット履歴を集める
- LLMで作業内容、技術領域、行為、必要スキルを分類する
- 分析結果をレトロスペクティブや評価材料として見られる形にする
