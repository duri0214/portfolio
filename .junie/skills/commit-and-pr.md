...
0. **本番環境の権限チェック (重要)**
    - 本番環境 (`ubuntu` ユーザー) でのデプロイやバッチ実行時に `PermissionError` が発生しないか確認します。
    - 特に `media/` 以下のファイル所有権 (`ubuntu:www-data`)、グループ書き込み権限 (`775/664`)、および ACL (`setfacl`) が適切に設定されているかを確認し、問題があれば `server-permission-management.md` に基づいて対処してください。
