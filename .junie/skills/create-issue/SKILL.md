...
課題や新機能、バグ修正などのタスクを管理するために、GitHub Issueを新規作成します。標準的なテンプレートに基づき、背景、目的、受け入れ条件などを整理して登録します。
...
3. **Issueの作成**
    ...
    - **優先度の確認**：UIの選択肢（低・中・高）から選択してもらい、以下のラベルを付与します。
        - `優先度: 低`
        - `優先度: 中`
        - `優先度: 高`
