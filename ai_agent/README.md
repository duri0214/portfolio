...
- 問題以外の2マスに「経験値ボーナス」と「休憩」を配置する
- 問題は単一教科、または国語×算数・国語×理科・算数×理科の科目横断に対応する
- プレイヤーは問題駒とプリセットセリフを選択する
- 経験値ボーナスへ移動すると、1回だけ経験値を10獲得する
- 休憩へ移動すると、1回だけ未解決の問題を1HP回復する（最大HP3）
- 成功した Skill は問題にダメージを与え、経験値を加算する
- 失敗した Skill は結果を残すが、問題と経験値は変更しない
- Agentの説明、選んだSkill、加工内容、結果、状態変化を実行履歴に残す
- 盤面イベントの移動先、効果、経験値・問題HPの変化も同じ履歴に残す

ゲーム状態は `GameState` の不変スナップショットで表し、`GameService` が問題選択、イベントマスへの移動、Skill適用を担当します。状態全体（盤面、HP、経験値、選択状態、Skill履歴、実行履歴）はDjangoセッションへ保存し、ゲーム状態本体をCookieへ保存しません。既存の旧ゲーム状態Cookieは読み込まず破棄し、必要に応じて初期状態から再開します。DBモデルや固定ターン制の履歴は使用しません。イベントマスの効果はそれぞれ1回だけ発生し、イベントマスへ移動すると問題・セリフの選択を解除します。
...
- `GameState`: 盤面、問題、イベントマス、経験値、選択状態、Tool履歴、ゲーム実行履歴のスナップショット
- `GameService`: ゲーム状態の生成、問題・イベント・セリフ選択、Skill適用
