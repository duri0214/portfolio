...
- 3x3 の盤面にプレイヤーと3体の敵駒を配置する
- 敵は国語、算数、理科のいずれかに対応する
- プレイヤーは敵駒とプリセットセリフを選択する
- 成功した Skill は敵にダメージを与え、経験値を加算する
- 失敗した Skill は結果を残すが、敵と経験値は変更しない
...
各Toolは `target_enemy_id` と `score` を構造化された入力として受け取り、成功・失敗、ダメージ、獲得経験値、敵の残りHPを構造化された辞書で返します。Toolの呼び出し順は画面側で固定せず、Agentが結果を見て次のToolを選択します。

## 主なクラス

- `GameState`: 盤面、敵、経験値、選択状態、Tool履歴のスナップショット
- `GameService`: ゲーム状態の生成、敵・セリフ選択、Skill適用
...
ドメインテストでは、初期盤面、敵・セリフ選択、単一Toolの成功・失敗、異なる順番のTool Chain、教科と敵の不一致、表示名と実装名の対応、SDK Function Tool登録を検証します。
