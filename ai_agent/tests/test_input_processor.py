...
class InputProcessorTest(TestCase):
    """
    InputProcessorのドメインサービステスト

    このテストクラスは、AI Agent システムの入力処理ドメインサービスの
    コアロジックをテストします。ガードレール機能、入力バリデーション、
    外部API統合（OpenAI）など、セキュリティと安全性に関わる重要な機能を検証します。

    ファイル配置について：
    このテストは ai_agent/tests/ に配置されています。tests/domain/service/ ではない理由：
    - Djangoモデル（Entity, GuardrailConfig）を使用するデータベース統合テスト
    - django.test.TestCase を継承（トランザクション管理、fixtureロードが必要）
    - 外部API（OpenAI）との統合を含む結合テスト
    - 純粋な単体テストではなく、データベース + 外部サービス連携込みのドメインサービステスト
    - Djangoプロジェクトでは「ドメインサービス + DB + 外部API統合テスト」は app/tests/ が適切

    テスト対象：
    - 入力処理パイプライン（前処理、検証、処理、後処理）
    - 静的ガードレール（文字数制限、禁止ワード、スパム検出）
    - 動的ガードレール（OpenAI Moderation API統合）
    - エンティティ固有の処理設定
    - エラーハンドリングとフォールバック処理
    - セキュリティ機能（XSS対策、入力無害化）

    テストの性質：
    - 統合テスト：データベース、外部API、ドメインサービスの統合
    - セキュリティテスト：悪意ある入力に対する防御機能の検証
    - パフォーマンステスト：処理時間の妥当性確認
    - モックテスト：外部API（OpenAI）の応答をモック化

    使用するfixture：
    - entity.json：テスト用エンティティとガードレール設定
    - AIアシスタント、セキュアAIアシスタントの2種類のエンティティ

    重要性：
    このテストは、ユーザー入力を安全に処理するための最初の防衛線として
    機能するため、セキュリティ上極めて重要な役割を果たします。

    他のテストクラスとの違い：

    vs test_conversation.py（ConversationServiceTest）：
    - このテスト：入力処理パイプライン、セキュリティ、ガードレール機能
    - ConversationServiceTest：エンティティ行動順序、タイムライン管理
    - 処理段階：入力前処理・検証 vs 会話フロー制御
    - 外部連携：OpenAI API統合 vs データベースのみ
    - セキュリティ焦点：悪意ある入力対策 vs システム動作保証

    vs test_next_turn_logic.py（NextTurnLogicTestCase）：
    - このテスト：単一エンティティでの入力→出力処理
    - NextTurnLogicTestCase：複数エンティティでの連続的な会話進行
    - スコープ：1リクエスト内の処理 vs 複数ターンにわたるシステム状態変化
    - テスト対象：入力安全性 vs 会話継続性
    - エンティティ使用：fixture設定のエンティティ vs プログラム生成エンティティ

    このテストの責務：
    ユーザー入力が安全かつ適切に処理され、システムセキュリティが
    保たれることを保証する（会話が始まる前の段階）
    """
