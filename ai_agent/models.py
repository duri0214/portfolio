...
class Review(models.Model):
    ...
    author_name = models.CharField(max_length=255)  # レビューの著者
    review_date = models.DateTimeField()  # レビューの日付
    latitude = models.FloatField()  # 場所の緯度
    longitude = models.FloatField()  # 場所の経度
    vector = models.BinaryField(null=True)  # ベクトル（Chromaで生成されたもの）
...
class Entity(models.Model):
    ...
    forbidden_keywords = models.TextField()  # 禁止ワードリスト (Cが使用)
    vector = models.BinaryField(null=True)  # ベクトル（Chromaで生成されたもの）
...
class Message(models.Model):
    ...
    conversation = models.ForeignKey(
        ConversationHistory, on_delete=models.CASCADE
    )  # 関連する会話
