...
class NextTurnView(View):
    ...
    def post(request, *args, **kwargs):
        ...
        3. エンティティの基本情報を取得
        4. エンティティに紐づくタイムラインが存在しない場合は重大なシステムエラー（データ不整合）
        5. Userエンティティの場合は操作を拒否（ボタンではなくチャットフォームからの入力が必要なため）
        6. エンティティが行動可能か確認し、行動不能な場合はその旨を通知して処理を中断
        7. ThinkingEngineProcessorを使用して応答を生成
        ...
        try:
            ...
            # 6. エンティティが行動可能か確認し、行動不能な場合はその旨を通知して処理を中断
            active_entity_timeline.can_act = TurnManagementService.can_respond_to_input(
                active_entity, ""
            )
            active_entity_timeline.save()
            if not active_entity_timeline.can_act:
                thinking_type_disp = active_entity.get_thinking_type_display()
                TurnManagementRepository.create_message(
                    content=f"[ERROR]{active_entity.name}（{thinking_type_disp}）はチャットに参加できませんでした",
                    action_history=current_action_history,
                )
                ...
                warning_msg = f"{active_entity.name}（{thinking_type_disp}）はチャットに参加できない状態です。"
            ...
            # 7. ThinkingEngineProcessorを使用して応答を生成
            ThinkingEngineProcessor.execute_thinking_engine(
                action_history=current_action_history,
