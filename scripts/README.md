...
2. バックアップ取得後、スクリプトが待機状態になります。
    - **💡 安全設計**: スクリプトは実行した瞬間の最新バックアップファイル名のみを記憶します。`scripts/backups`
      フォルダに過去のファイルが残っていても、それらを二重に取り込んだり誤って復元することはありません。
3. 別のターミナル等で `python manage.py migrate` を実行し、DBをリセットします。
4. 元のスクリプトに戻り、Enter を押すと手順2で記憶したデータのみが復元されます。
...
2. 各アプリの Fixture ロードと初期バッチが順番に実行されます。
    - Taxonomy は fixture ではなく seed コマンドで初期データを投入します。
    - ※ このスクリプト内でダミーデータ生成 (`generate_industry_dummy_data`) も行われます。Step 1
      で復元したデータがある場合は上書きに注意してください。

Shopping画面だけを目検する場合は、全アプリの初期投入を行わず、次のコマンドで画面確認用データを投入できます。

```powershell
python manage.py seed_shopping_data
```

このコマンドは既存の店舗・ユーザー・商品を削除せず、同じキーで再実行しても重複しません。
...
5. **Taxonomy**: seed コマンドによる生物分類体系・LLM生成済み動物分類候補・卵管理
